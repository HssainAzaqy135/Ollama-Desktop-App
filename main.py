...
print('Booting Chat...', end='')
response = ollama.chat(model=model, messages=[{'role':'user', 'content':"Don't reply to this message"}])
print('\r' + curr_Chat.title)
i = 1
while(True):
    ...
    util.print_response_tab(model_name = model,response= response['message']['content'],index= i)
